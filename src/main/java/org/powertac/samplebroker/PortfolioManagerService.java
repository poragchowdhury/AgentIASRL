...
implements PortfolioManager, Initializable, Activatable
{
  ...
  private double defaultMargin = 0.3;

  @ConfigurableValue(valueType = "Double",
          description = "Fixed cost/kWh")
  private double fixedPerKwh = -0.05;
          ...
          if (null == oldc) {
            ...
            TariffSpecification spec =
              new TariffSpecification(brokerContext.getBroker(),
                  ...
                  .withPeriodicPayment(defaultPeriodicPayment * 1.0);
            Rate rate = new Rate().withValue(rateValue);
            spec.addRate(rate);
            if (null != oldc)
...
//  }
  ...
  class CustomerRecord
  {
    ...
    double getTariffPrice (int index)
    {
      if (index < 0) {
        log.warn("usage requested for negative index " + index);
        index = 0;
